...
public class MessageUtils {
    ...
    public static Integer ProcessingOut2ErrorOUT(@NotNull MessageQueueVO messageQueueVO, @NotNull MessageDetails4Send messageDetails, TheadDataAccess theadDataAccess,
    ...
    public static int XML_Current_Tags4Send(@NotNull MessageDetails4Send messageDetails, int Current_Elm_Key) throws UnsupportedOperationException, NullPointerException  {
            ...
            if (ChildVO_ArrayList != null ) { // у элемента <messageDetailVO.Tag_Id ...> есть либо атрибуты, либо вложенные элементы
                ...
                while (ChildVO_ElementIterator.hasNext()) {
                    ...
                    if (messageChildVO != null) {
                        ...
                        {  // вызываем рекурсию
                            XML_Current_Tags4Send(messageDetails, i);
                        }
                    }
                    else {
    ...
    public static Long readMessage_QueueVO(long Queue_Id, MessageQueueVO messageQueueVO, MessageDetails4Send Message_4_Send, TheadDataAccess theadDataAccess, boolean IsDebugged, Logger MessegeReceive_Log ){
        ResultSet rs;
        try {
            theadDataAccess.stmtMsgQueueVO_Query.setLong(1, messageQueueVO.getQueue_Id() );
            rs = theadDataAccess.stmtMsgQueueVO_Query.executeQuery();
    ...
    public static String PrepareEnvelope4XSLTPost4Send( MessageQueueVO messageQueueVO, @NotNull MessageDetails4Send messageDetails, Logger MessegeSend_Log) {
    ...
    public static Integer ProcessingSendError(@NotNull MessageQueueVO messageQueueVO, @NotNull MessageDetails4Send messageDetails, TheadDataAccess theadDataAccess,
    ...
    public static Integer ProcessingOutError(@NotNull MessageQueueVO messageQueueVO, @NotNull MessageDetails4Send messageDetails, TheadDataAccess theadDataAccess,
                                             String whyIsFault , Exception e , Logger MessegeSend_Log)
    {
        String ErrorExceptionMessage;
        if ( e != null ) {
    ...
    public static int SplitMessage4Send(MessageDetails4Send messageDetails, Element EntryElement, int tag_Par_Num,
        ...
        for (int i = 0; i < Elements.size(); i++) {
            ...
            SplitMessage4Send(messageDetails, XMLelement,
    ...
    public static int ReplaceMessageOnly4Send(@NotNull TheadDataAccess theadDataAccess, long Queue_Id, @NotNull MessageDetails4Send messageDetails, Logger MessegeReceive_Log) {
        int nn = 0;
    ...
    public static int SplitConfirmation(MessageDetails4Send messageDetails, Element EntryElement, int tag_Par_Num,
                                        Logger MessegeSend_Log) {
        // Tag_Par_Num- №№ Тага, к которому прилепляем всё от EntryElement,ссылка на Tag_Num родителя
        // Tag_Num - сквозной!!! нумератор записей
    ...
    public static int   ReplaceConfirmation(@NotNull TheadDataAccess theadDataAccess, long Queue_Id, @NotNull MessageDetails4Send messageDetails, Logger MessegeSend_Log) {
        int nn = 0;
        ...
        try {
            for ( iNumberRecordInConfirmation = 0; iNumberRecordInConfirmation < messageDetails.Confirmation.size(); iNumberRecordInConfirmation++) {
                ...
                //theadDataAccess.stmt_INSERT_Message_Details.setString(3, MessageDetailVO.Tag_Value);
                if ( MessageDetailVO.Tag_Value.length() > (XMLchars.MAX_TAG_VALUE_BYTE_SIZE /2) ) {
                    String ElementContentS = new String( XMLchars.cutUTF8ToMAX_TAG_VALUE_BYTE_SIZE(org.apache.commons.lang3.StringEscapeUtils.unescapeHtml4(MessageDetailVO.Tag_Value)), StandardCharsets.UTF_8 );
                    theadDataAccess.stmt_INSERT_Message_Details.setString(3, ElementContentS );
                }
                else {
                    theadDataAccess.stmt_INSERT_Message_Details.setString(3, org.apache.commons.lang3.StringEscapeUtils.unescapeHtml4(MessageDetailVO.Tag_Value) );
                }
    ...
    public static int ReplaceMessage4SEND(TheadDataAccess theadDataAccess, long Queue_Id, @NotNull MessageDetails4Send messageDetails, MessageQueueVO messageQueueVO , Logger MessegeSend_Log) {
        int nn = 0;
        ...
        try {
            ...
            SplitMessage4Send(messageDetails, RootElement, 0, // Tag_Num = messageDetails.Message_Tag_Num !
                    MessegeSend_Log);
        ...
        nn = MessageUtils.ReplaceMessageOnly4Send(theadDataAccess, Queue_Id, messageDetails, MessegeSend_Log);
...
*/
    ...
    public static String PrepareEnvelope4ErrTransXSLT( MessageQueueVO messageQueueVO, @NotNull MessageDetails4Send messageDetails, Logger MessegeSend_Log) {
        int nn = 0;
        StringBuilder SoapEnvelope = new StringBuilder(XMLchars.Envelope_noNS_Begin);
        SoapEnvelope.append(XMLchars.Header_noNS_Begin);
        SoapEnvelope.append("<MsgId>" + messageQueueVO.getQueue_Id() +"</MsgId>");
    ...
    public static int ReadMessageDetai4Send(TheadDataAccess theadDataAccess, long Queue_Id, @NotNull MessageDetails4Send messageDetails, boolean IsDebugged, Logger MessegeSend_Log) {
        messageDetails.Message.clear();
        messageDetails.MessageRowNum = 0;
        messageDetails.Message_Tag_Num = 0;
    ...
    public static String PrepareConfirmation(TheadDataAccess theadDataAccess, MessageQueueVO messageQueueVO, @NotNull MessageDetails4Send messageDetails, Logger MessegeSend_Log) {
        int nn = 0;
                ...
                try {
                    ...
                    if ( emtNext != null ) {
                        ...
                        theadDataAccess.doUPDATE_MessageQueue_Send2finishedOUT(messageQueueVO.getQueue_Id(), AnswXSLTQueue_Direction,
                                org.apache.commons.lang3.StringEscapeUtils.unescapeHtml4(messageDetails.MsgReason.toString()),
                                iMsgStaus,
